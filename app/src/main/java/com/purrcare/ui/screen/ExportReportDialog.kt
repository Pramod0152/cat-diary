...
fun ExportReportDialog(
    ...
    AlertDialog(
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                when (state) {
                    ...
                    is ReportViewModel.ReportState.Error -> {
                        ...
                        Text(
                            "Generate a PDF report for ${catProfile.name} with weight history, daily logs, and medication adherence.",
                            style = MaterialTheme.typography.bodyMedium
                        )
        ...
        confirmButton = {
            if (state !is ReportViewModel.ReportState.Generating) {
                Button(
                    onClick = { viewModel.generateReport(catProfile, selectedDays) },
